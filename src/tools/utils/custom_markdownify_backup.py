...
class MarkdownConverter(object):
    ...
    def convert_table(self, el, text, parent_tags):
        # Process the table to get better formatting
        processed_table = self._format_table(el, text)
        return '\n\n' + processed_table + '\n\n'
    ...
    def convert_td(self, el, text, parent_tags):
        ...
        if nested_table:
            # Extract the nested table as a separate markdown table
            # Store it separately and replace with a placeholder
            nested_md = self._convert_nested_table(nested_table)
            if hasattr(self, '_nested_tables'):
                self._nested_tables.append(nested_md)
            else:
    ...
    def _convert_nested_table(self, table_el):
        ...
        temp_converter = MarkdownConverter(**self.options)
        return temp_converter.convert(str(table_el)).strip()
    ...
    def convert_tr(self, el, text, parent_tags):
        # Store row data for later table formatting
        if not hasattr(self, '_current_table_rows'):
        ...
        for cell in cells:
            # Get cell text content, excluding nested tables
            cell_copy = cell.__copy__()
            # Remove nested tables from the copy to avoid double processing
            for nested_table in cell_copy.find_all('table'):
                nested_table.decompose()
            
            cell_text = self.process_tag(cell_copy, parent_tags={'_inline'}).strip()
            # Clean up the text - remove excessive whitespace and newlines
            cell_text = ' '.join(cell_text.split())
            
            # Handle empty cells - use a non-breaking space to preserve structure
            if not cell_text:
                cell_text = ' '
        ...
        if any(cell.strip() for cell in row_data):
            is_header = (
                any(cell.name == 'th' for cell in cells) or 
                el.parent.name in ['thead', 'th']
            )
        ...
        return ''  # Return empty, formatting handled in _format_table

    def _format_table(self, el, text):
        """Format table with dynamic column widths and text wrapping"""
        if not hasattr(self, '_current_table_rows') or not self._current_table_rows:
            return text.strip()
        
        rows = self._current_table_rows
        self._current_table_rows = []  # Clear for next table
        ...
        non_empty_rows = []
        for row in rows:
            if any(cell.strip() and cell.strip() != ' ' for cell in row['data']):
        ...
        # Calculate max columns, but ignore trailing empty columns
        max_cols = 0
        for row in rows:
        ...
        # Truncate all rows to max_cols and pad if necessary
        for row in rows:
            row['data'] = row['data'][:max_cols]
            while len(row['data']) < max_cols:
                row['data'].append('')
        
        # Calculate optimal column widths
        max_width = self.options.get('table_max_width', 100)
        min_col_width = self.options.get('table_min_col_width', 8)
        max_col_width = self.options.get('table_max_col_width', 30)
        ...
        for col_idx in range(max_cols):
            ...
            for row in rows:
                if col_idx < len(row['data']):
                    content = row['data'][col_idx].strip()
                    if content and content != ' ':
                        col_content_widths.append(len(content))
            
            if col_content_widths:
                ideal_width = max(col_content_widths)
                col_widths.append(max(min_col_width, min(ideal_width, max_col_width)))
            else:
                col_widths.append(min_col_width)
        
        # Adjust widths if total exceeds max_width
        total_width = sum(col_widths) + (max_cols - 1) * 3 + 4  # 3 chars for " | ", 4 for borders
        if total_width > max_width:
            # Proportionally reduce column widths
            available_width = max_width - (max_cols - 1) * 3 - 4
        ...
        for row in rows:
            formatted_cells = []
            for col_idx in range(max_cols):
                cell_content = row['data'][col_idx].strip() if col_idx < len(row['data']) else ''
                ...
                # Wrap text if needed
                if len(cell_content) > cell_width:
                    wrapped_text = cell_content[:cell_width-3] + '...'
                else:
                    wrapped_text = cell_content
                
                # Pad to column width
                formatted_cells.append(wrapped_text.ljust(cell_width))
            
            formatted_row = '| ' + ' | '.join(formatted_cells) + ' |'
            formatted_rows.append(formatted_row)
            
            # Add separator after header
            if row['is_header'] and not header_found:
                separator_cells = ['-' * w for w in col_widths]
                separator_row = '| ' + ' | '.join(separator_cells) + ' |'
                formatted_rows.append(separator_row)
                header_found = True
        
        # If no header was found, add one at the beginning
        if not header_found and formatted_rows:
            separator_cells = ['-' * w for w in col_widths]
            separator_row = '| ' + ' | '.join(separator_cells) + ' |'
            formatted_rows.insert(1, separator_row)  # Insert after first row to make it header
...
def custom_markdownify(html, **options):
    """Custom markdownify that handles nested tables and better table formatting"""
    # Set default table formatting options
    default_options = {
        'table_max_width': 120,
    ...
    return MarkdownConverter(**default_options).convert(html)
