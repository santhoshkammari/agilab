...
run_name = "epoch-16-fd-sw-auto-loss-480-ftdataset-pubtabdataset"
...
# Load your datasets
ds1 = load_dataset("ds4sd/FinTabNet_OTSL")
ds2 = load_dataset("/home/ng6309/datascience/santhosh/datasets/PubTabNet_OTSL_full")

# First, check the structure of both datasets
logger.info("FinTabNet splits:", ds1.keys())
logger.info("PubTabNet splits:", ds2.keys())

# Check if columns match
logger.info("\nFinTabNet columns:", ds1['train'].column_names)
logger.info("PubTabNet columns:", ds2['train'].column_names)

# Combine the datasets
ds = DatasetDict({
    "train": concatenate_datasets([ds1["train"], ds2["train"]]),
    "val": concatenate_datasets([ds1["val"], ds2["val"]]),
    "test": ds1["test"]  # Assuming only FinTabNet has test split
})

logger.info("\nCombined dataset:")
logger.info(ds)
logger.info(f"Train size: {len(ds['train'])}")
logger.info(f"val size: {len(ds['val'])}")
logger.info(f"Test size: {len(ds['test'])}")

# Save the combined dataset
ds.save_to_disk("/home/ng6309/datascience/santhosh/datasets/Combined_OTSL")
