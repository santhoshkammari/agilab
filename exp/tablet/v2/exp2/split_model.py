...
class TableDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        h_gt_960, v_gt_960 = get_ground_truth_auto_gap_expand_min5pix_overlap_cells(
...
def evaluate_model(model, val_dataloader, device, logger=None):
    ...
    with torch.no_grad():
        for batch_idx, (images, h_gt_480, v_gt_480, h_gt_960, v_gt_960) in enumerate(val_dataloader):
            ...
            # Calculate metrics at 960 resolution
            h_pred_binary = (h_pred_960 > 0.5).float()
            v_pred_binary = (v_pred_960 > 0.5).float()

            # Accuracy at 960
            h_accuracy = (h_pred_binary == h_gt_960).float().mean()
            v_accuracy = (v_pred_binary == v_gt_960).float().mean()
            h_accuracy_sum += h_accuracy.item()
            v_accuracy_sum += v_accuracy.item()

            # Precision and Recall (for split regions) - use 960 resolution
            h_tp = ((h_pred_binary == 1) & (h_gt_960 == 1)).sum().float()
            h_fp = ((h_pred_binary == 1) & (h_gt_960 == 0)).sum().float()
            h_fn = ((h_pred_binary == 0) & (h_gt_960 == 1)).sum().float()

            v_tp = ((v_pred_binary == 1) & (v_gt_960 == 1)).sum().float()
            v_fp = ((v_pred_binary == 1) & (v_gt_960 == 0)).sum().float()
            v_fn = ((v_pred_binary == 0) & (v_gt_960 == 1)).sum().float()
...
def visualize_predictions(model, dataset, device, idx=0, threshold=0.5):
    ...
    # get one sample - now returns 5 items
    image_tensor, h_gt_480, v_gt_480, h_gt_960, v_gt_960 = dataset[idx]
    ...
    # convert predictions to binary at 960
    h_binary = (h_pred > threshold).astype(int)
    v_binary = (v_pred > threshold).astype(int)

    # use 960 GT for comparison
    h_gt_np = h_gt_960.numpy()
    v_gt_np = v_gt_960.numpy()
...
def train_split_model(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, (images, h_gt_480, v_gt_480, h_gt_960, v_gt_960) in enumerate(train_dataloader):
            images = images.to(device)
            h_gt_480, v_gt_480 = h_gt_480.to(device), v_gt_480.to(device)
