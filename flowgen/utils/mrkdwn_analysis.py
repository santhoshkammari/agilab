...
class MarkdownParser:
    ...
    def parse_indented_code_block(self):
        ...
        if lines:
            content = "\n".join(lines)
            end_line = self.pos  # Current position is the line after the code block
            self.tokens.append(BlockToken('code', content=content, meta={"language": None, "code_type": "indented", "end_line": end_line}, line=start+1))
        ...
        for token in self.tokens:
            if token.type == 'code':
                result["Code block"].append({
                    ...
                    "end_line": token.meta.get("end_line"),
