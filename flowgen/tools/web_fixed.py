...
class PlaywrightBrowser:
    ...
    async def initialize(self):
        ...
        logger.debug(f"Page creation took: {time.time() - page_start:.3f}s")
...
class WebSearchTool:
    ...
    async def web_search(self, query: str, search_provider: str = 'bing', num_results: int = 10) -> str:
...
async def _get_browser():
    ...
    if _browser_instance is None:
        _browser_instance = PlaywrightBrowser(headless=False)
        await _browser_instance.initialize()
    return _browser_instance

def search_web(query: str, max_results: int = 5) -> str:
    """Sync wrapper that properly handles async context"""
    try:
        # Check if there's already an event loop running
        loop = asyncio.get_running_loop()
        # If we're in an async context, we need to handle this differently
        raise RuntimeError("Cannot call search_web from within async context")
        ...
        if "no running event loop" in str(e):
            ...
            # We're in an async context, raise the error
            raise e
...
if __name__ == '__main__':
    # Test with multiple searches using the same browser
