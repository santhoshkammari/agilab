...
SYSTEM_PROMPT = """You are a markdown analysis assistant. You have access to the following tools for analyzing markdown content:
...
Each tool takes markdown content as a string parameter and returns data with line numbers for easy navigation. Use these tools to analyze markdown content and provide insights about their structure and content."""


def _get_markdown_analyzer(content: str):
    print(f"DEBUG: Creating MarkdownAnalyzer for content ({len(content)} chars)")
    import time
    start = time.time()
...
def markdown_analyzer_get_headers(content: str):
    ...
    try:
        analyzer = _get_markdown_analyzer(content)
...
def markdown_analyzer_get_overview(content: str):
    ...
    try:
        ...
        intro_info = {
            ...
            "content_preview": intro_result.get('content', '')[:150] + '...' if isinstance(intro_result, dict) and intro_result.get('content') else 'No introduction found'
        }
...
- **Preview**: {overview_data['introduction']['content_preview']}
...
"""
    ...
    messages = [
        ...
        {"role": "user", "content": "analyze this markdown content and show me the structure"}
    ]
...
"""
    ...
    res = markdown_analyzer_get_code_blocks(sample_content)
