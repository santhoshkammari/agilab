...
class vLLM(BaseLLM):
    ...
    def chat(self, input, **kwargs) -> dict:
        ...
        # Handle streaming response - check if it's a stream object
        if hasattr(response, '__class__') and 'Stream' in str(response.__class__):
            # This is a streaming response - collect all chunks
            content_parts = []
            tool_calls_dict = {}  # Track tool calls by ID to accumulate chunks
            ...
            for chunk in response:
                if hasattr(chunk, 'choices') and chunk.choices:
                    ...
                    if hasattr(choice, 'delta') and choice.delta:
                        ...
                        # Handle tool calls - accumulate streaming chunks
            ...
            result['think'] = ""  # vLLM doesn't have reasoning_content in streaming
            
            # Convert accumulated tool calls to list
            if tool_calls_dict:
                result['tool_calls'] = list(tool_calls_dict.values())
                # Ensure arguments is valid JSON or empty dict
...
class vLLMAsync(BaseLLM):
    def __init__(self, host: str = "0.0.0.0", port="8000", model=None, tools=None, format=None, timeout=None, api_key="dummy"):
        super().__init__(model=model, tools=tools, format=format, timeout=timeout)
        self._client = AsyncOpenAI(api_key=api_key, base_url=f"http://{host}:{port}/v1")
...
class OllamaAsync(BaseLLM):
    def __init__(self, host: str = "localhost", port: str = "11434", model=None, tools=None, format=None, timeout=None):
        super().__init__(model=model, tools=tools, format=format, timeout=timeout)
        from ollama import AsyncClient
        self._client = AsyncClient(host=f"http://{host}:{port}", timeout=timeout)
    ...
    async def chat(self, input, **kwargs) -> dict:
        ...
        options = {**kwargs}
        response = await self._client.chat(model=model, messages=input,
                                     format=format,
                                     tools=tools,
                                     options=options)
        result = {"think": "", "content": "", "tool_calls": None}
